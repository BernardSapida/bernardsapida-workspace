...
import DownloadableTemplate from "./_components/DownloadableTemplate";
import EmptySection from "./_components/EmptySection";
import Footer from "./_components/Footer";
import Header from "./_components/Header";
import Outputs from "./_components/Outputs";
import BusinessPlanLoadingSkeleton from "./_components/LoadingBusinessPlans";
const InputsAndAnalysis = lazy(() => import("./_components/InputsAndAnalysis"));
...
const BusinessPlanPage: FunctionComponent<BusinessPlanPageProps> = ({
  ...
  const shouldShowEmpty = currentStep.sections.length === 0;
  const shouldShowTabs =
    currentStep.sections.length > 0 && currentStepNumber !== 14;
  ...
  useEffect(() => {
    if (businessPlan) setValues(businessPlan.values as Record<string, any>);
  }, [businessPlan]);
    ...
    >
      ...
      <div className="flex-1 flex flex-col min-h-screen relative">
        <div className="flex-1 px-6 sm:px-8 lg:px-10 py-8">
          ...
          {/* Download Business Plan Template */}
          <Activity mode={shouldShowEmpty ? "visible" : "hidden"}>
            <EmptySection />
          </Activity>
